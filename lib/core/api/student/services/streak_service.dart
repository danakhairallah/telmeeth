...
class StreakServices {
  ...
  Future<RestoreStreakModel> restoreStreak(int lessonId) async {
    try {
      ...
      final response = await dio.post("/student/restore-streak/$lessonId");

      if (response.statusCode == 200) {
        return RestoreStreakModel.fromJson(response.data);
      } else {
        return RestoreStreakModel(
          message: "Failed to restore streak (Status code: ${response.statusCode})",
        );
      }
    } catch (e) {
      print("Restore streak error: $e");
      return RestoreStreakModel(
        message: "Failed to restore streak due to error",
      );
    }
  }
}
...
import '../model/request/interactive_lesson_submit_request.dart';
...
class InteractiveLessonService {
  ...
  Future<InteractiveLessonGetModel?> getInteractiveLessonById(int id) async {
    try {
      final dio = await ApiClient.getDio();

      final response = await dio.get(
        "/student/interactive-lessons/$id",
      );

      if (response.statusCode == 200) {
        return InteractiveLessonGetModel.fromJson(response.data); // مباشرة
      }

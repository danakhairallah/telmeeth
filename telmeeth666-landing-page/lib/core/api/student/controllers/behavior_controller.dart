...
class BehaviorController with ChangeNotifier {
  ...
  int get positiveCount =>
      response?.behaviors?.where((b) => b.type == 'positive').length ?? 0;

  int get negativeCount =>
      response?.behaviors?.where((b) => b.type == 'negative').length ?? 0;

  int get neutralCount =>
      response?.behaviors?.where((b) => b.type == 'neutral').length ?? 0;
}
